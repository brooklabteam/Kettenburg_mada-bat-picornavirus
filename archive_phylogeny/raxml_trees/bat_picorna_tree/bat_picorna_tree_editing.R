...
p2 <- ggtree(rooted.tree) %<+% tree.dat + geom_tippoint(aes(color=Species, shape=Host), size=4,stroke=0,show.legend = T) +
  ...
  theme(legend.position = "none", 
        ...
        legend.key.size = unit(0.3, "cm")) +
  xlim(c(0,5))+
  geom_cladelabel(node = 48, label = "Shanbavirus A (collapsed)",offset=0.05, fontsize=4, color="black")+
  geom_cladelabel(node = 45, label = "Shanbavirus A (collapsed)",offset=0.05, fontsize=4, color="black")+
  geom_cladelabel(node = 70, label = "Shanbavirus A (collapsed)",offset=0.05, fontsize=4, color="black")
p2
...
p3<-collapse(p2.1, 48)+geom_point2(aes(subset=(node==48)), size=4, shape=22, fill="white")
p4<-collapse(p3, 45)+geom_point2(aes(subset=(node==45)), size=4, shape=22, fill="white")
